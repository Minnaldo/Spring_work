...
<body>
	...
	<br />
	<div align="center">
	<form action="main.do?action=date" method="post">
		오늘 시킨다면 언제 제품이 오는지 알고싶으신가요?
		<input type="text" name="posx" value="사는곳의 X좌표">
		<input type="text" name="posy" value="사는곳의 Y좌표">
	...
	<div style="margin-left: 200px">
		<table id="content" style="width: 90%; margin: 0 auto">
			...
			<tr style='margin-left:200px'><td>
			<a href='main.do?action=pdetail&code=${product.code	 }'>
			<img width='180px'  src = "${product.image}"   class='btn btn-gray-transparent btn-animated btn-sm'></a>
			...
			<c:if test="${fn:contains(alcheck, a)}"><h4 style="color:red">${a} 알러지가 포함되어 있는 상품입니다!!!!</h4>
			</c:if>
			</c:forEach>
			<br>
			<span class ='glyphicon glyphicon-calendar'> Feb, 2017 </span>&emsp;
			<span class ='glyphicon glyphicon-tag'>Web Design</span><br/>
			<hr><p style='opacity:0.6'>${product.material }</p>
